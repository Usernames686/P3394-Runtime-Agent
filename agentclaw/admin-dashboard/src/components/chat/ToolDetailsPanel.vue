<template>
  <div class="tool-details-panel" :class="{ show: visible }">
    <div class="details-header mono-font">
      ...
      <button class="details-close" type="button" aria-label="Close details" @click="$emit('close')">x</button>
    </div>
    <div class="details-body mono-font">
      <template v-if="isGroup">
        <div v-for="(item, index) in tool.tools" :key="item.id || index" class="tool-call-section">
          ...
          <CommandResultCard v-if="commandResultFor(item)" :tool="item" />
          <template v-else>
      ...
      <template v-else>
        <CommandResultCard v-if="commandResultFor(tool)" :tool="tool" />
        <template v-else>
          <div class="log-section">
            <span class="log-label">[Input]</span>
...
export default {
  ...
  computed: {
    ...
    panelTitle() {
      if (this.isGroup) return `${this.tool.name} x${this.tool.tools.length}`
  ...
  methods: {
    ...
    formatDuration(tool) {
      if (tool?.duration_ms !== undefined && tool?.duration_ms !== null) return `${Number(tool.duration_ms).toFixed(0)}ms`
      return tool?.elapsed || ''
    },
    commandResultFor(tool) {
      return normalizeCommandResult(tool)
    },
  },
}
</script>
