...
const baseMenuItems = computed(() => [
  { label: t('nav.p3394Agent'), key: '/p3394-agent', icon: 'P' },
  { label: t('nav.memoryGraph'), key: '/memory-graph', icon: 'G' },
  { label: t('nav.builtin'), key: '/builtin', icon: 'A' },
  { label: t('nav.templates'), key: '/templates', icon: 'T' },
])
...
const activeKey = computed(() => {
  ...
  if (path === '/' || path.startsWith('/p3394-agent')) return '/p3394-agent'
  if (path.startsWith('/memory-graph')) return '/memory-graph'
