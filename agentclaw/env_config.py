...
ENV_SECTIONS: tuple[EnvSection, ...] = (
    ...
    EnvSection(
        ...
        variables=(
            ...
            EnvVarSpec("AGENTCLAW_ENABLE_ADMIN_API", "true", "是否挂载 Admin API；默认开启以保持本地开发体验。"),
            EnvVarSpec("AGENTCLAW_ENABLE_DASHBOARD", "true", "是否挂载 Dashboard 静态页面；默认开启以保持本地开发体验。"),
            EnvVarSpec("AGENTCLAW_DASHBOARD_MODE", "full", "Dashboard 模式：full 或 public-chat；public-chat 只服务公开聊天页 SPA 入口。"),
            EnvVarSpec("AGENTCLAW_ENABLE_MCP_ROUTES", "true", "是否挂载 /mcp* 路由；公开演示部署可关闭。"),
            EnvVarSpec("AGENTCLAW_ENABLE_SCHEDULER_API", "true", "是否挂载 /api/scheduler* 路由；公开演示部署可关闭。"),
            EnvVarSpec("AGENTCLAW_ENABLE_CHANNEL_ROUTES", "true", "是否挂载 /api/channels* 路由；公开演示部署可关闭。"),
            EnvVarSpec("AGENTCLAW_ENABLE_API_DOCS", "true", "是否暴露 /docs、/redoc、/openapi.json；公开演示部署可关闭。"),
            EnvVarSpec("AGENTCLAW_CORS_ORIGINS", "*", "CORS 允许来源，逗号分隔；默认保持本地开发宽松行为。"),
            EnvVarSpec("AGENTCLAW_CORS_ALLOW_CREDENTIALS", "true", "CORS 是否允许 credentials；默认保持当前行为。"),
    ...
    EnvSection(
        ...
        variables=(
            ...
            EnvVarSpec("AGENTCLAW_PUBLIC_MAX_TTS_CHARS", "2000", "Public 文本转语音最大文本字符数。"),
            EnvVarSpec("AGENTCLAW_PUBLIC_MAX_INPUT_BYTES", "16384", "Public workflow 单次 JSON 请求体最大字节数；仅影响匿名公开工作流。"),
            ...
            EnvVarSpec("AGENTCLAW_PUBLIC_TOOL_POLICY", "allow", "Public workflow 工具策略：allow、block_builtin 或 block_all；默认 allow。"),
            EnvVarSpec("AGENTCLAW_PUBLIC_ALLOWED_BUILTIN_TOOLS", "", "AGENTCLAW_PUBLIC_TOOL_POLICY=block_builtin 时仍允许的内置工具名，逗号分隔。"),
        ),
    ),
    EnvSection(
